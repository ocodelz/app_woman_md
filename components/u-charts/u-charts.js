...
function createCurveControlPoints(points, i) {
	...
	function isNotMiddlePointX(points, i) {
		if (points[i - 1] && points[i + 1]) {
			return points[i].x >= Math.max(points[i - 1].x, points[i + 1].x) || points[i].x <= Math.min(points[i - 1].x, points[
				i + 1].x);
		} else {
			return false;
		}
...
function getTouches(touches, opts, e) {
	...
	if (touches.clientX) {
		if (opts.rotate) {
			...
			x = (touches.pageY - e.currentTarget.offsetTop - (opts.height / opts.pixelRatio / 2) * (opts.pixelRatio - 1)) * opts
				.pixelRatio;
		} else {
			x = touches.clientX * opts.pixelRatio;
			y = (touches.pageY - e.currentTarget.offsetTop - (opts.height / opts.pixelRatio / 2) * (opts.pixelRatio - 1)) * opts
				.pixelRatio;
		}
	} else {
		if (opts.rotate) {
...
function drawMarkLine(opts, config, context) {
	...
	// console.log(markLineOption)
	if (points.length === 2) {
		context.beginPath();
		context.setFillStyle(hexToRgb(markLineOption.areacolor,markLineOption.areaopacity));
		let firstPoint = points[0]
		let lastPoint = points[1]
		context.moveTo(startX, firstPoint.y)
		context.lineTo(endX, firstPoint.y)
		context.lineTo(endX, lastPoint.y)
		context.lineTo(startX, lastPoint.y)
		context.moveTo(startX, firstPoint.y)
		context.closePath()
		context.fill()
	}
...
function drawLineDataPoints(series, opts, config, context) {
	...
	series.forEach(function(eachSeries, seriesIndex) {
		...
		let shadowBlur = eachSeries.shadowBlur ? eachSeries.shadowBlur : false
		if (shadowBlur) {
			context.shadowColor = eachSeries.color;
